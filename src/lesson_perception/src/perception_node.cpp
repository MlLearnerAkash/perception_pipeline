    ...
    private:
        ...
        void cloud_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr recent_cloud)
        {
             ...
             * ========================================*/
              ...
              pcl::PointCloud<pcl::PointXYZ>::Ptr xf_cloud;
              pcl::PassThrough<pcl::PointXYZ> xf_pass;
              xf_pass.setInputCloud(cloud_voxel_filtered);
              xf_pass.setFilterFieldName("x");
              xf_pass.setFilterLimits(-1.0, 1.0);
              xf_pass.filter(*xf_cloud);

              //for y-axis filtered
              pcl::PointCloud<pcl::PointXYZ>::Ptr xyf_filtered_cloud;
              pcl::PassThrough<pcl::PointXYZ> yf_pass;
              yf_pass.setInputCloud(xf_cloud);
              yf_pass.setFilterFieldName("y");
              yf_pass.setFilterLimits(-1.0, 1.0);
              yf_pass.filter(*xyf_filtered_cloud);
              
              //for y-axis filtered
              pcl::PointCloud<pcl::PointXYZ>::Ptr xyz_filtered_cloud;
              pcl::PassThrough<pcl::PointXYZ> zf_pass;
              zf_pass.setInputCloud(xyf_filtered_cloud);
              zf_pass.setFilterFieldName("z");
              zf_pass.setFilterLimits(-1.0, 1.0);
              zf_pass.filter(*xyz_filtered_cloud);
            /* ========================================
            ...
            this->publishPointCloud(voxel_grid_publisher_, *xyz_filtered_cloud);
